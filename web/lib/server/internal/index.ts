...
export async function isCurrentUserOwner(projectId: string) {
    ...
    const { is_owner } = await db.selectNoFrom(({ exists, selectFrom }) => exists(
        selectFrom("projects")
...
export async function queueWorkflow(workflowId: string, {
    ...
    if (!workflow.is_enabled)
        throw new CodedError("Workflow is disabled", 400)
...
export class WorkflowRunLimitExceededError extends CodedError {
    usage?: number
    limit?: number

    constructor(usage?: number, limit?: number) {
        super(
            `Plan limit exceeded${(usage && limit) ? ` (${usage} / ${limit} workflow runs)` : ""}`,
            429
        )
        this.usage = usage
        this.limit = limit
