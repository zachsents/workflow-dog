...
import { internalsSymbol, useReactFlow, useStore } from "reactflow"
...
export default function GhostBuster() {
    ...
    const handleMap = useStore(state => Object.fromEntries(
        Array.from(state.nodeInternals.values()).map(node => {
            const handleBounds = node[internalsSymbol]?.handleBounds
            return [
                node.id,
                [
                    ...(handleBounds?.source?.map(h => h.id) ?? []), 
                    ...(handleBounds?.target?.map(h => h.id) ?? []),
                ],
            ]
        })
    ), _.isEqual)
