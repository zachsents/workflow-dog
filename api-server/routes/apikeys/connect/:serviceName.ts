...
import { checkForErrorThenJson, defaultApiKeyAccountConfig, fetchProfile } from "@api/util.js"
...
export async function post(req: Request, res: Response) {
    ...
    const config = merge({}, defaultApiKeyAccountConfig, baseConfig)
    ...
    const profile = await fetch(config.profileUrl, {
        headers: {
            Authorization: config.generateAuthHeader(req.body.apiKey),
        },
